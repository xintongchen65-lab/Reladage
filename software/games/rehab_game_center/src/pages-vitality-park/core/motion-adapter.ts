import type { RepEvent, TrainingState } from '../../game-platform/motion/types'
import type { RawVitalityMotionFrame, VitalityMotionFrame, VitalityMotionStage } from '../types/motion'

export type VitalityRejectReason =
...
const stages: VitalityMotionStage[] = ['SITTING', 'LEAN_FORWARD', 'LIFT_OFF', 'HALF_STANDING', 'STANDING', 'SIT_BACK']

export type VitalityAdaptResult =
  | { accepted: true; frame: VitalityMotionFrame; diagnostics: string[] }
  | { accepted: false; reason: VitalityRejectReason }

export class VitalityMotionFrameAdapter {
  private last: VitalityMotionFrame | null = null
  reset(): void { this.last = null }
  getLastSeq(): number { return this.last?.seq ?? -1 }

  ingest(raw: unknown): VitalityAdaptResult {
    let value = raw
    ...
    const frame = value as VitalityMotionFrame
...
function isRawFrame(value: unknown): value is RawVitalityMotionFrame {
  if (!value || typeof value !== 'object') return false
  const frame = value as Record<string, unknown>
  const numeric = ['seq', 'timestamp_ms', 'left_angle_deg', 'right_angle_deg', 'left_rom_deg', 'right_rom_deg', 'lr_rom_diff_deg', 'left_count', 'right_count', 'target_count', 'completion_percent', 'left_speed_deg_s', 'right_speed_deg_s', 'set_index', 'target_sets', 'rest_remaining_sec', 'target_angle_deg', 'valid_angle_deg', 'return_angle_deg', 'overall_completion_percent']
  if (frame.mode !== 'lower' || frame.body_mode !== 'lower' || frame.exercise !== 'sit_to_stand') return false
  if (!['none', 'sit_to_stand_done'].includes(String(frame.rep_event))) return false
  if (!states.includes(frame.training_state as TrainingState)) return false
  ...
  if (frame.motion_stage !== undefined && !stages.includes(frame.motion_stage as VitalityMotionStage)) return false
  if (numeric.some((key) => typeof frame[key] !== 'number' || !Number.isFinite(frame[key]))) return false
  if (frame.motion_progress !== undefined && (typeof frame.motion_progress !== 'number' || !Number.isFinite(frame.motion_progress) || frame.motion_progress < 0 || frame.motion_progress > 1)) return false
  return Number(frame.seq) >= 0 && Number(frame.timestamp_ms) >= 0 && Number(frame.target_count) > 0 && Number(frame.set_index) >= 1 && Number(frame.target_sets) >= Number(frame.set_index) && Number(frame.return_angle_deg) < Number(frame.valid_angle_deg) && Number(frame.valid_angle_deg) <= Number(frame.target_angle_deg) && Number(frame.completion_percent) >= 0 && Number(frame.completion_percent) <= 100 && Number(frame.overall_completion_percent) >= 0 && Number(frame.overall_completion_percent) <= 100
}

export function eventForFrame(frame: VitalityMotionFrame): boolean { return (frame.rep_event as RepEvent) === 'sit_to_stand_done' }
