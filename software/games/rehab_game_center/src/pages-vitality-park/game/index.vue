<template>
  <view class="vp-game">
    ...
    <view class="vp-game__hud" :style="{ right: `${hudSafeRight}px` }">
...
export default {
  ...
  methods: {
    refreshPlatformInsets() {
      let safeRight = 12
      /* #ifdef MP-WEIXIN */
      try {
        const system = (uni as any).getSystemInfoSync?.() ?? {}
        const menu = (uni as any).getMenuButtonBoundingClientRect?.()
        if (menu && Number.isFinite(menu.left) && Number.isFinite(system.windowWidth)) {
          safeRight = Math.max(12, Math.ceil(system.windowWidth - menu.left + 8))
        } else {
          safeRight = 104
        }
      } catch {
