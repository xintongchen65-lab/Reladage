<template>
  <view class="rfg-game">
    <view class="rfg-game__stage" :style="stageCssVars">
...
import { gameStageStyle, getViewportLayout } from '../runtime/viewport-layout'
...
export default {
  ...
  computed: {
    ...
    stageCssVars(): Record<string, string> {
      return gameStageStyle(this.viewportLayout)
    },
    stageSafeTopPx(): number {
      return this.viewportLayout.safeTopPx / Math.max(0.001, this.viewportLayout.stageScale)
    },
...
.rfg-game__stage { position: relative; flex: 0 0 auto; width: 1280px; height: 720px; max-width: none; max-height: none; overflow: hidden; background: #8ed65b; }
...
.rfg-game__target { position: absolute; top: 20%; top: max(20%, calc(var(--rfg-safe-top, 8px) + 48px)); z-index: 10; display: flex; flex-direction: column; align-items: center; width:104px;width: clamp(104px, 14vw, 170px); transition: left 180ms ease, right 180ms ease; }
...
.rfg-game__target-label { position: relative; z-index: 2; margin-top: -5px; padding: 4px 11px; border: 2px solid #c97815; border-radius: 999px; background: #fff3ae; color: #743c0b; font-size:11px;font-size: clamp(11px, 1.8vh, 15px); font-weight: 900; line-height: 1.2; white-space: nowrap; box-shadow: 0 3px 0 rgba(103, 54, 8, 0.25); }
.rfg-game__bonus { position: absolute; left: 50%; top: 17%; top: max(17%, calc(var(--rfg-safe-top, 8px) + 45px)); z-index: 16; display: flex; flex-direction: column; align-items: center; width: 17%; transform: translateX(-50%); animation: rfg-bonus-enter 350ms ease-out; }
...
.rfg-game__bonus-time, .rfg-game__bonus-label { padding: 2px 9px; border-radius: 999px; background: rgba(255, 245, 181, 0.95); color: #8a4009; font-size:10px;font-size: clamp(10px, 1.7vh, 13px); font-weight: 900; white-space: nowrap; }
...
.rfg-game__basket-label { margin-top: -4px; padding: 3px 10px; border-radius: 999px; background: rgba(255, 246, 204, 0.95); color: #74420d; font-size:10px;font-size: clamp(10px, 1.7vh, 13px); font-weight: 900; box-shadow: 0 3px 0 rgba(93, 53, 8, 0.28); }
.rfg-game__basket-button::after { border: 0; }
.rfg-game__feedback { position: absolute; left: 50%; bottom: 2.5%; z-index: 18; box-sizing: border-box; max-width: 42%; padding: 5px 18px; overflow: hidden; border-radius: 999px; background: rgba(255, 249, 214, 0.94); color: #70400c; font-size:11px;font-size: clamp(11px, 1.9vh, 16px); font-weight: 800; line-height: 1.25; white-space: nowrap; text-overflow: ellipsis; transform: translateX(-50%); }
...
.rfg-game__effect-score { position: absolute; left: 50%; bottom: 5%; color: #fff; font-size:24px;font-size: clamp(24px, 5.5vh, 42px); font-weight: 900; text-shadow: 0 3px 0 #e66c0f, 0 0 8px #ffef62; transform: translateX(-50%); }
...
.rfg-game__pause-title { color: #fff; font-size:34px;font-size: clamp(34px, 8vh, 62px); font-weight: 900; text-shadow: 0 5px 0 #35591c; }
.rfg-game__pause-message { margin-top: 10px; color: #fff8c8; font-size:14px;font-size: clamp(14px, 2.5vh, 20px); font-weight: 700; }
...
.rfg-game__diagnostics { position: absolute; left: 1.5%; bottom: calc(1.5% + 38px); z-index: 40; padding: 3px 8px; border-radius: 6px; background: rgba(0, 0, 0, 0.5); color: #fff; font-size:10px;font-size: clamp(10px, 1.6vh, 13px); }
...
/* Shared 1280x720 stage overrides: stable on phones, tablets and desktop previews. */
.rfg-game__target { top: 20%; width: 170px; }
.rfg-game__bonus { top: 17%; }
.rfg-game__target-label { font-size: 15px; }
.rfg-game__bonus-time, .rfg-game__bonus-label, .rfg-game__basket-label { font-size: 13px; }
.rfg-game__feedback { font-size: 16px; }
.rfg-game__effect-score { font-size: 42px; }
.rfg-game__pause-title { font-size: 58px; }
.rfg-game__pause-message { font-size: 20px; }
.rfg-game__diagnostics { font-size: 13px; }
</style>
